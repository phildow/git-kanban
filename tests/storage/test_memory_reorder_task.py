"""Tests for InMemoryRepository.reorder_task."""
...
class TestInMemoryReorderTask(unittest.TestCase):
    ...
    def _setup_three(self) -> tuple[Task, Task, Task]:
        """Create Alpha, Beta, Gamma in insertion order."""
    ...
    def test_other_tasks_unaffected(self) -> None:
        ...
        t1, t2, t3 = self._setup_three()
        self.repo.reorder_task(t2, "top")
        self.assertEqual(self.repo.get_task("proj", "todo", "alpha").title, "Alpha")
        self.assertEqual(self.repo.get_task("proj", "todo", "gamma").title, "Gamma")
    ...
    def test_single_task_up_is_noop(self) -> None:
        ...
        task = self.repo.get_task("proj", "todo", "alpha")
    ...
    def test_raises_task_not_found_for_unknown_task(self) -> None:
        """Raises TaskNotFound when the task is not in the store."""

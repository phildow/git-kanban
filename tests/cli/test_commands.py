...
class TestCommandHandlers(unittest.TestCase):
    ...
    def test_handle_task_assign(self):
        ...
        args = self._args(path="board-a/todo/fix-parser", assigned_to="alice", remove=False)
        result = object()
    ...
    def test_handle_task_assign_with_remove_flag(self):
        """`task assign --remove` clears the task's assigned_to via unset_task."""
        args = self._args(path="board-a/todo/fix-parser", assigned_to=None, remove=True)
        result = object()
        self.svc.unset_task.return_value = result

        commands.handle_task_assign(args, self.svc, self.renderer, self.json_renderer)

        self.svc.unset_task.assert_called_once_with(Path("/board-a/todo/fix-parser"), TaskUnsetParams(assigned_to=True))
    ...
    def test_handle_task_tag(self):
        ...
        args = self._args(path="board-a/todo/fix-parser", tags="auth", remove=False)
        result = object()
    ...
    def test_handle_task_tag_with_remove_flag(self):
        """`task tag --remove` forwards path and tag to untag_task and renders result."""
        args = self._args(path="board-a/todo/fix-parser", tags="auth", remove=True)
        result = object()
